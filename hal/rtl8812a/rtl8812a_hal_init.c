...
static inline BOOLEAN Hal_EfuseSwitchToBank8812A(
    IN		PADAPTER	pAdapter,
    IN		u1Byte		bank,
...
static void hw_var_set_monitor(PADAPTER Adapter, u8 variable, const u8 *val)
{
	...
	if (*((const u8 *)val) == _HW_STATE_MONITOR_) {
...
static void hw_var_set_mlme_sitesurvey(PADAPTER Adapter, u8 variable, const u8* val)
{
	...
	if(*((const u8 *)val)) { //under sitesurvey
